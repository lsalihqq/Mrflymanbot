...
if is_module_loaded(FILENAME):
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("Kullanımını etkinleştirdi `{}`".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
    ...
    def __stats__():
        return "{}devre dışı bırakılmış öğeler {} sohbet.".format(sql.num_disabled(), sql.num_chats())
    ...
    __mod_name__ = "Komut devre dışı bırakma"
